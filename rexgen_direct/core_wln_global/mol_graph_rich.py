...
def smiles2graph(smiles, idxfunc=lambda x:x.GetIdx()):
    ...
    fatoms = np.zeros((n_atoms, atom_fdim))         # 原子特征
    fbonds = np.zeros((n_bonds, bond_fdim))         # 键特征
    atom_nb = np.zeros((n_atoms, max_nb), dtype=np.int32)   # 表示每个原子连接的原子是啥
    bond_nb = np.zeros((n_atoms, max_nb), dtype=np.int32)   # 表示每个原子连接的键是啥
    num_nbs = np.zeros((n_atoms,), dtype=np.int32)      # 表示每个原子连了多少个键

    degree=np.zeros((n_atoms,n_atoms))       # 增加了度矩阵
    ...
    return fatoms, fbonds, atom_nb, bond_nb, num_nbs, degree

def pack2D(arr_list):      # 多余的部分用0填充
...
def smiles2graph_list(smiles_list, idxfunc=lambda x:x.GetIdx()):
    ...
    fatom_list, fbond_list, gatom_list, gbond_list, nb_list, de_list= zip(*res)
    return pack2D(fatom_list), pack2D(fbond_list), pack2D_withidx(gatom_list), pack2D_withidx(gbond_list), pack1D(nb_list), pack2D(de_list), get_mask(fatom_list)
# get_mask 掩码：表示多少个原子有效

m = Chem.MolFromSmiles('CC')
assignProperties(m)
atom = m.GetAtoms()[0]
bond = m.GetBonds()[0]
atom_fdim = len(atom_features(atom))    # 原子特征维度
bond_fdim = len(bond_features(bond))    # 键特征维度
